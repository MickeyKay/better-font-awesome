...
The server reads the current site's `better-font-awesome_options.default_block_icon_style` setting, validated through the existing Settings API and AJAX handlers. Both the renderer and editor resolve inheritance from the active validated BFAL catalog. Shortcodes, Classic Editor insertion, raw HTML, and third-party icons are unaffected. Automatic and local delivery, deliberate earlier BFAL ownership, metadata HTTP, cron, and asset loading retain their established behavior.
