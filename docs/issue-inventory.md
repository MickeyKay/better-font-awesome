...
| --- | --- | --- | --- | --- |
| BFA-001 | P0 | Live release data is stale or unavailable | Shipped in 2.1, monitor | The former path produced repeated [403 API failures](https://wordpress.org/support/topic/error-fetching-data-from-api-2/), [429 failures](https://wordpress.org/support/topic/429-too-many-requests-7/), remote CSS failures, and GitHub [#27](https://github.com/MickeyKay/better-font-awesome/issues/27). Public BFA 2.1 and BFAL 2.1 use validated asynchronous stale-while-revalidate Font Awesome 5 metadata with locking, backoff, persistent last-known-good data, and a bundled fallback. Current development extends that architecture to a default Font Awesome 7 channel with a packaged fallback, but it is not yet in a public BFA release. Monitor public behavior before further architecture changes. |
...
| BFA-004 | P1 | Native block editor experience | Implemented in development, pending public release | The API v3 dynamic icon block uses `block.json`, semantic icon attributes, existing PHP rendering, accessible decorative and labelled modes, searchable Free icons, and editor browser tests. Alignment refinement and any inline-icon expansion remain separate follow-up decisions. |
...
| BFA-010 | P3 | Font Awesome Pro and custom kits | Deferred | Pro and kits add authentication, licensing, private registry, and support complexity. Revisit after the current Free update path and native block have public release evidence and stable behavior. |
...
3. Refine alignment and other approved native block behavior while preserving the implemented semantic and shortcode-compatible foundation.
4. Monitor the implemented Font Awesome 7 Free path and investigate any separate Font Awesome 6 requirement only from concrete compatibility evidence.
