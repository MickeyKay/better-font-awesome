...
5. Persists the complete durable replacement only when the initially observed record is still absent or exactly unchanged. A concurrent valid record wins and is reloaded; a concurrent invalid or unresolved value leaves migration incomplete and retryable.
...
Focused follow-up regressions model WordPress boot before overdue cron dispatch, repeated boot calls, missing and newly claimed markers, malformed ownership, site-only activation, network activation, new sites in two networks, and blog-context restoration. They also model expired and replaced worker leases during remote callbacks, stale success and failure callbacks, atomic lease validation and renewal, concurrent transient migration, failed failure-state persistence, and external object-cache transients whose original expiration is unavailable or contradicted by a leftover database timeout. These tests prove that an exact overdue event is preserved while only a missing old event is recovered, inactive multisite sites receive no metadata work, demonstrated remote-callback ownership loss cannot overwrite replacement results, concurrent migration preserves the winning valid record, failed state persistence publishes no immediate retry, and external-cache migration schedules exactly one background refresh without request-path metadata HTTP.
