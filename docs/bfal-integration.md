...
BFA requires a reviewed public stable BFAL release that supplies the corrected `get_asset_delivery()` contract. Dependency identity, public archive verification, and comparison with the previously integrated implementation belong in the release PR. Stable rollback compatibility remains intact. See the [release procedure](release-procedure.md) for strict Composer validation and the remaining release checks.
